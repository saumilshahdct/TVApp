...
class APIRepository : BaseDataSource() {
	...
	fun authenticationPolling(deviceCode: String) = performOperation(APIConstants.authenticationPolling) {
		getResult(APIConstants.authenticationPolling) {
			APIUtil.service.authenticationPolling(AppConstants.grantType, deviceCode, AppConstants.clientId, AppConstants.clientSecret)
		}
	}
	...
	fun fetchUserStats(userStatsAPIURL: String, eventIds: String) = performOperation(APIConstants.fetchUserStats) {
	...
	fun fetchEntityDetails(entity: String, entityId: String) = performOperation(APIConstants.fetchEntityDetails) {
		getResult(APIConstants.fetchEntityDetails) {
			APIUtil.service.fetchEntityDetails(entity, entityId)
		}
	}

	fun fetchEntityUpcomingEvents(entityScope: String) = performOperation(APIConstants.fetchEntityUpcomingEvents) {
		getResult(APIConstants.fetchEntityUpcomingEvents) {
			APIUtil.service.fetchEntityUpcomingEvents(entityScope)
		}
	}

	fun fetchEntityOnDemandEvents(entityScope: String) = performOperation(APIConstants.fetchEntityOnDemandEvents) {
		getResult(APIConstants.fetchEntityOnDemandEvents) {
			APIUtil.service.fetchEntityOnDemandEvents(entityScope)
		}
	}

	fun fetchEntityPastEvents(entityScope: String) = performOperation(APIConstants.fetchEntityPastEvents) {
	...
	fun addRemoveWatchListEvent(eventId: HashMap<String, Any>, isRemoveFromWatchList: Boolean) = performOperation(APIConstants.watchListEvents) {
		getResult(APIConstants.watchListEvents) {
	...
	fun fetchEventStreamDetails(eventId: String) = performOperation(APIConstants.fetchEventStreamDetails) {
	...
	fun fetchEventProductDetails(eventId: String) = performOperation(APIConstants.fetchEventProductDetails) {
		getResult(APIConstants.fetchEventProductDetails) {
			APIUtil.service.fetchEventProductDetails(eventId)
		}
	}

	fun claimFreeTicketForEvent(eventId: String) = performOperation(APIConstants.claimFreeTicketForEvent) {
	...
	fun setNewReservation(itemId: HashMap<String, Any>) = performOperation(APIConstants.setNewReservation) {
	...
	fun createOrder(orderDetails: HashMap<String, Any>) = performOperation(APIConstants.createOrder) {
